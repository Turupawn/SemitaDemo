                               ...
                               ARMarkerInfo2 *marker_info2, int *marker_num )
{
    ...
    for (i = j = 0; i < *marker_num; i++) {
        ...
        if (arGetLine(marker_info2[i].x_coord, marker_info2[i].y_coord,
                      ...
                      marker_infoL[j].line, marker_infoL[j].vertex) < 0 ) {
#ifdef DEBUG_LOGGING
        __android_log_print(ANDROID_LOG_INFO,"AR","arDetectMarker->arGetMarkerInfo->arGetLine couldn't fit line to num_coords: %d",marker_info2[i].coord_num);
#endif
		continue;
	}
