...
int arParamDisp( ARParam *param )
{
    ...
    printf("--------------------------------------\n");
    __android_log_print(ANDROID_LOG_INFO,"AR","--------------------------------------");
    printf("SIZE = %d, %d\n", param->xsize, param->ysize);
    __android_log_print(ANDROID_LOG_INFO,"AR","SIZE = %d, %d\n", param->xsize, param->ysize);
    printf("Distortion factor = %f %f %f %f\n", param->dist_factor[0],
            param->dist_factor[1], param->dist_factor[2], param->dist_factor[3] );
    __android_log_print(ANDROID_LOG_INFO,"AR","Distortion factor = %f %f %f %f\n", param->dist_factor[0],
            param->dist_factor[1], param->dist_factor[2], param->dist_factor[3] );
    for( j = 0; j < 3; j++ ) {
        for( i = 0; i < 4; i++ ) {
		printf("%7.5f ", param->mat[j][i]);
	}
	__android_log_print(ANDROID_LOG_INFO,"AR","%7.5f %7.5f %7.5f %7.5f", 
	param->mat[j][0], param->mat[j][1], param->mat[j][2], param->mat[j][3]);	
        printf("\n");
    }
    __android_log_print(ANDROID_LOG_INFO,"AR","--------------------------------------");
    printf("--------------------------------------\n");
